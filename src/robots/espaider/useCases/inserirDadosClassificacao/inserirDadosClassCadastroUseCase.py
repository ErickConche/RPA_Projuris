    ...
    def execute(self):
        try:
            ...
            *** Informações Tópico Geral ***
            ...
            time.sleep(5)
            select_option(page=self.page, name="RegistroNivel1Edt", value=self.data_input.unidade_controle)
            time.sleep(1)
            ...
            if 'trabalhista' in self.data_input.natureza.lower():
                ...
                time.sleep(4)
                select_option(page=self.page, name="CLI_TipoNatureza", value=self.data_input.tipo)
                time.sleep(5)
            self.iframe.query_selector('[name="Tipo"]').type(self.data_input.tipo_acao.split('  ')[0])
            time.sleep(2)
            select_option(page=self.page, name="Tipo", value=self.data_input.tipo_acao)
            time.sleep(2)
